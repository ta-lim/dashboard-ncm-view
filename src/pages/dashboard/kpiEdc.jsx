...
import getKpiEdc from "@/api/activity/getKpiEdc";





export function KpiEdc() {
  ...
  async function getAllData() {
    const res = await getKpiEdc(spesificKpi)
    if (res) {
      if (res.status === "200") {
        // let remappedData = remapData(res.data);
        // setData(remappedData);
    ...
    return ({
      ...
      options: {
        ...
        title: {
          ...
          text: data.category !== undefined ?data.category[index]["categoryWilayahOperasional"] : ""
        },
        dataLabels: {
          enabled: true,
          enabledOnSeries: [1]
  ...
  const KPI = ["Ketersedian EDC", "Implementasi EDC"];
                ...
                }
                {
                  isLogin && (
                    <div className={`flex items-start md:flex-1`}>
                      <Link to={`./upload/${spesificKpi === 0 ? "availibility-edc" : spesificKpi === 1 ? "implementation-edc" : null}`}>
                        <IconButton
                          size="sm"
                          variant="text"
                          color="blue-gray"
            ...
            >
              {
                data.length !== 0 && data.datasets.map((item, key) => {
                  ...
                  return (
                    ...
                    {data.length !== 0 && <Chart className='grow px-8' {...chartConfig(item)} />}
                    </Card>
                  );
                })
              }
