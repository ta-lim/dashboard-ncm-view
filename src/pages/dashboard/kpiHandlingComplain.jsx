...
import Kpi from "./kpi";
import getKPIHandlingComplain from "@/api/activity/getKpiHandlingComplain";





export function KpiHandlingComplain() {
  ...
  const categoryKpi = spesificKpi === 0 ? "categoryMachine" : null; 
  ...
  async function getAllData() {
    const res = await getKPIHandlingComplain(spesificKpi)
    if (res) {
      if (res.status === "200") {
        // let remappedData = remapData(res.data);
        // setData(remappedData);
  ...
  // }
    ...
    return ({
      ...
      options: {
        ...
        title: {
          text: spesificKpi == 1 ? KPI[spesificKpi] : data.category !== undefined ? data.category[1]["categoryMachine"] : ""
        },
        dataLabels: {
          enabled: true,
          enabledOnSeries: [1]
  ...
  const KPI = ["Handling Complain", "Dispute Resolution"];
                ...
                }
                {
                  isLogin && (
                    <div className={`flex items-start md:flex-1`}>
                      <Link to={`./upload/${spesificKpi === 0 ? "handling-complain" : spesificKpi === 1 ? "dispute-resolution" : null }`}>
                        <IconButton
                          size="sm"
                          variant="text"
                          color="blue-gray"
                  ...
                  data.length !== 0 && data.datasets.map((item, key) => {
                    ...
                    return (
                      <Card className=" w-full py-3 px-12">
                        {data.length !== 0 && <Chart className='grow px-8' {...chartConfig(item, key)} />}
                      </Card>
                    );
                  })
                }
